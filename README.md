...
5. 문서 담당 AI는 실제 사용자 승인 근거와 범위를 확인하고 승인된 분석·정비만 진행합니다. 하위 구현 위임까지 승인됐고 원본 전수 등록·소유권·버전 차이·기존 구현 감사·정책 Ref 준비가 끝난 경우에만 실제 구현을 하위 카드 AI에 위임합니다. 그룹→문서 위임은 분석·조정 전용으로 worker를 점유하지 않습니다.
6. 그룹 개요에서 문서별 AI 상태, 하위 업무 집계, 위임·복구 상태와 결과를 확인합니다. 하위 업무가 남아 있으면 그룹 위임은 문서 검수까지 기다린 뒤 총괄 AI에 결과를 전달합니다. 결과 회수·자동 재개·복구는 새로운 실행 승인이 아니며, 승인된 계획 안의 남은 작업만 이어갑니다. 방향·범위·기획 기준 변경이나 승인 근거가 불명확한 경우 사용자에게 재제안합니다.

승인 근거는 사용자가 승인한 제안·대상·허용 작업과 실제 승인 발언 및 확인 가능한 대화·메시지 출처를 함께 전달합니다. 총괄 AI가 문서 AI의 실행을 스스로 승인하거나 승인 근거를 추측해서 만들면 안 됩니다. 미승인 제안은 대화로만 보고하며 카드·댓글·공유 지식·대기 항목을 변경하지 않습니다. 제안 작성이나 위임 응답이 끝났다는 사실은 개발 완료나 다음 실행 승인이 아닙니다.

현재 승인 절차는 AI 전문의 행동 규칙이며 서버가 승인 토큰을 검사해 변경·위임 API를 거부하는 강제 승인 기능은 아닙니다. 별도의 승인 버튼·승인 대기 상태 저장도 추가하지 않았습니다. 새 전문 적용에는 서버·MCP 재시작/재연결과 화면 새로고침이 필요하며, 이미 실행 중인 대화·전달된 전문·대기열 작업·기존 카드 본문은 자동 변경하거나 중지하지 않습니다. 기존 대화를 계속 사용할 때는 새 승인 규칙을 명시적으로 전달하고 최신 그룹 문맥을 다시 조회해야 합니다.
...
- 작업은 완료됐으나 상위 AI 전달만 실패한 경우 `작업 완료 · 총괄 보고 대기`로 표시합니다. 결과 전달 재시도는 하위 작업을 재실행하지 않으며, 완료된 하위 업무를 보고 실패만으로 미완료 집계하지 않습니다.
- 문서 조정 실행 자체가 끝났지만 의도적으로 남긴 하위 업무·외부 대기 때문에 `waiting-document-work`에 머무르면 `대기 유지하고 조정 종료`로 카드와 하위 위임을 그대로 보존한 채 현재 결과만 총괄 AI에 전달할 수 있습니다. 재시작 뒤 과거 operation이 사라져 `recovery-required`로 후퇴한 구버전 기록도 결과 해시가 보존되어 있으면 같은 방법으로 복구합니다. MCP에서는 사용자의 명시적인 요청 후 `mindnprogress_finalize_ai_coordination`을 사용합니다.
...
| `mindnprogress_finalize_ai_coordination` | 완료된 문서 조정 실행이 하위 업무·문서 검수 대기에 남았을 때, 사용자 요청으로 카드 상태·진행률·외부 대기와 하위 위임을 보존한 채 조정 위임만 종료하고 현재 결과를 상위 AI에 전달합니다. |
