...
3. `총괄 AI 대화 시작`에서 원본 분석과 문서 분할을 요청합니다. 총괄 AI는 각 문서 루트에 담당 범위·분석 순서·정책·완료 조건을 기록하고 문서 담당 AI에 위임할 수 있습니다.
4. 문서 담당 AI는 분석과 기존 구현 감사를 거쳐 하위 업무를 구성하고, 실제 구현을 하위 카드 AI에 위임합니다. 그룹→문서 위임은 분석·조정 전용으로 worker를 점유하지 않습니다.
5. 그룹 개요에서 문서별 AI 상태, 하위 업무 집계, 위임·복구 상태와 결과를 확인합니다. 하위 업무가 남아 있으면 그룹 위임은 문서 검수까지 기다린 뒤 총괄 AI에 결과를 전달합니다.
...
서브 머신의 Runner가 접속하려면 메인 머신의 API가 LAN에서 열려 있어야 합니다. `MNP_API_HOST`가 기본값 `127.0.0.1`이면 어떤 Runner도 연결할 수 없으므로 `0.0.0.0`으로 지정해 실행합니다. 루프백에만 바인딩된 상태라면 분산 작업 설정 화면이 이 사실을 먼저 알려 줍니다.

1. 메인 머신 화면에서 계정 메뉴 → `분산 작업 설정`을 엽니다. 메인 머신은 서버가 기동할 때 자동으로 등록되어 있습니다.
2. `서브 머신 등록`에 머신 ID와 이름을 입력합니다. 머신 ID는 소문자, 숫자, 하이픈만 사용합니다. **등록한 사람이 소유자가 되며 위임은 본인 소유 머신에만 보낼 수 있습니다.**
3. 등록한 머신의 `토큰 발급`을 누릅니다. 화면에 이 머신의 **Runner 실행 명령이 실제 접속 주소와 토큰까지 채워진 형태로** 표시되므로 `실행 명령 복사`를 누릅니다. 토큰은 이때 한 번만 표시되고 서버에는 해시만 저장되므로, 창을 닫은 뒤에는 다시 발급해야 합니다.
4. `서브 머신에 작업 분산 사용`을 켜고 기본 실행 머신을 고른 뒤 저장합니다.
5. 서브 머신의 저장소 폴더에서 복사한 명령을 실행합니다.
6. 머신 목록에서 `연결됨`으로 바뀌는지 확인합니다. `연결 확인`을 누르면 Runner를 거쳐 그 머신의 AionUi까지 실제로 닿는지 확인합니다.
...
서브 머신에 저장소를 복제하고 AionUi를 실행한 뒤 다음처럼 시작합니다. 별도 의존성 설치는 필요하지 않습니다.

```bash
MNP_RUNNER_API_URL=http://<메인 PC 주소>:4176 \
MNP_RUNNER_MACHINE_ID=macbook \
MNP_RUNNER_TOKEN=mnprn_... \
node runner/index.mjs
```

`npm run runner`로도 실행할 수 있습니다. 위 값은 분산 작업 설정의 `실행 명령 복사`로 그대로 얻을 수 있으므로 직접 입력할 필요는 없습니다.

| 환경변수 | 기본값 | 설명 |
| --- | --- | --- |
| `MNP_RUNNER_API_URL` | 필수 | 메인 머신의 MindNProgress API 주소 |
