...
3. 등록한 머신의 `토큰 발급`을 누릅니다. 토큰은 이때 한 번만 표시되므로 복사해 둡니다. 서버에는 해시만 저장됩니다.
4. `서브 머신에 작업 분산 사용`을 켜고 기본 실행 머신을 고른 뒤 저장합니다.
5. 서브 머신에서 Runner를 실행합니다.
6. `연결 확인`을 눌러 Runner를 거쳐 그 머신의 AionUi까지 닿는지 확인합니다.
...
`npm run runner`로도 실행할 수 있습니다. 메인 머신의 서버가 LAN에서 접근 가능해야 하므로 `MNP_API_HOST`를 `0.0.0.0`으로 지정해 실행합니다.
