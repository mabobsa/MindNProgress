...
const productGuide = {
  ...
  operationRules: [
    ...
    'coordination-only 위임의 실행이 완료됐지만 하위 업무·문서 검수 대기에 남았거나 그 상태가 재시작 뒤 recovery-required로 후퇴했고 보존 결과가 있으며, 사용자가 현재 미완료 상태와 외부 대기를 보존한 종료를 명시적으로 요청한 경우에만 mindnprogress_finalize_ai_coordination을 사용함. 이 도구는 카드 상태·진행률·대기 항목과 하위 위임을 변경하거나 하위 AI를 재실행하지 않고 현재 조정 결과의 상위 보고만 진행함',
...
async function main() {
  ...
  registerTool(server, 'mindnprogress_finalize_ai_coordination', '사용자가 명시적으로 요청한 경우에만, 실행 자체는 끝났지만 하위 업무·문서 검수 대기로 남았거나 재시작 뒤 recovery-required로 후퇴했으나 결과가 보존된 coordination-only 위임을 종료하고 현재 결과를 상위 AI에 전달합니다. 카드 상태·진행률·외부 대기와 하위 위임은 완료하거나 해제하지 않고 그대로 보존하며 하위 AI를 재실행하지 않습니다. 일반 구현 위임이나 결과가 없는 실행 중 위임에는 사용할 수 없습니다.', {
    mapId: z.string().min(1).describe('이 위임을 시작한 상위 문서 ID'),
