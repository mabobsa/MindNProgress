...
async function main() {
  ...
  try {
    ...
    assert.match(guide.routes.edit, /get_context/)
    assert.match(guide.routes.explore, /list_documents.*get_document/)
    assert.match(toolDescription('mindnprogress_complete_ai_delegation'), /사용자의 중지.*최종 답변 직전/)
    assert.match(toolDescription('mindnprogress_finalize_ai_coordination'), /사용자가 명시.*coordination-only/)
    ...
    assert.match(toolDescription('mindnprogress_checkpoint_ai_workspace'), /commit-changes.*confirm-no-changes.*검증 전에/)
    assert.match(toolDescription('mindnprogress_checkpoint_ai_workspace'), /응답.*HEAD/)
    assert.deepEqual(commitCheckpointSchema?.properties?.commitMessage?.required, ['summary', 'background', 'cause', 'changes'])
    assert.match(toolDescription('mindnprogress_delegate_ai_work'), /같은 문서의 하위 카드.*최신 conversationAssessmentId.*reasonCode\/message/)
    assert.match(toolDescription('mindnprogress_update_card'), /checklist.*전체 배열/)
    ...
    assert.doesNotMatch(JSON.stringify(guide.guide), /# 사용자 승인과 실행 범위|# 그룹의 두 단계 사용자 승인/)
    ...
    assert.match(toolDescription('mindnprogress_manage_comment'), /\[진행\].*\[차단\].*\[결과\].*list_comments/)
    assert.match(guide.guide.workPolicy, /체크리스트.*progress.*status/)
    ...
    assert.equal(context.selection.taskLinks.startupInspection.mode, 'linked-targets')
    ...
    assert.match(childDelegationInstruction, /후보 조회 → contextHealth 평가 확인 → resume이면 최신 conversationAssessmentId 전달/)
    assert.match(childDelegationInstruction, /reasonCode\/message 처리/)
    assert.match(childDelegationInstruction, /waiting·recovery 상태는 새 위임을 만들지 말고 statusTool과 recoveryTool/)
    assert.match(childDelegationInstruction, /최신 카드와 실제 산출물을 검증/)
    ...
    assert.match(context.nextStep, /selection\.taskLinks\.startupInspection.*selection\.workflow\.writePolicy.*guide\.contextLifecycle\.writeSafety/)
    ...
    assert.match(mockAionUi.dispatchRequests[0].instruction, /제안에 그치지 말고 맡긴 범위를 수행/)
    ...
    assert.match(mockAionUi.dispatchRequests[0].instruction, /응답을 받지 못한 시도만 다시 호출/)
    assert.doesNotMatch(mockAionUi.dispatchRequests[0].instruction, /mindnprogress_complete_ai_delegation/)
    ...
    await invokeExpectError('mindnprogress_supersede_ai_delegation', { mapId, delegationId: recoveredRun.delegation.id, replacementDelegationId: recoveredRun.delegation.id, expectedUpdatedAt: recoveredCurrent.updatedAt }, /후속 위임|상태가 변경/)
    ...
    assert.match(mockAionUi.dispatchRequests[1].instruction, /원래 지시를 반복하지 말고 미완료 부분만/)
    assert.match(mockAionUi.dispatchRequests[1].instruction, /맡긴 범위 밖 변경은 상위 AI에 보고/)
    assert.doesNotMatch(mockAionUi.dispatchRequests[1].instruction, /mindnprogress_complete_ai_delegation/)
    ...
    assert.equal(savedGroup.guide.documentCoordinator, undefined)
    assert.equal(savedGroup.guide.role, 'group-coordinator')
    ...
    assert.match(sourceContext.guide.sources, /project\.sources의 모든 기획서/)
    assert.match(sourceContext.guide.documentInstruction, /승인된 문서별 범위.*문서 루트 AI/)
    ...
    assert.match(groupCoordinatorContext.groupProject.instruction, /mindnprogress_get_group_context의 현재 역할 guide/)
    assert.doesNotMatch(groupCoordinatorContext.groupProject.instruction, /# 사용자 승인과 실행 범위|# 그룹의 두 단계 사용자 승인/)
    assert.doesNotMatch(JSON.stringify(groupCoordinatorContext.guide), /# 사용자 승인과 실행 범위|# 그룹의 두 단계 사용자 승인/)
    assert.equal(groupCoordinatorContext.selection.role.id, 'group')
    assert.equal(groupCoordinatorContext.selection.workflow.writePolicy, 'approval-required')
    assert.match(groupCoordinatorContext.nextStep, /selection\.workflow\.writePolicy/)
    ...
    assert.match(groupCoordinatorContext.selection.aiWorkCoordination.childDelegation.instruction, /같은 문서의 하위 카드/)
    ...
    for (const value of [groupDocumentContext.groupProject.instruction, JSON.stringify(groupDocumentContext.guide), groupDocumentContext.nextStep, groupDocumentContext.selection.aiWorkCoordination.childDelegation.instruction]) {
      assert.doesNotMatch(value, /# 사용자 승인과 실행 범위|# 그룹의 두 단계 사용자 승인|미승인 분석·제안은|사용자 승인 근거와 허용 범위가 확인된/)
    }
    assert.equal(groupDocumentContext.selection.role.id, 'document')
    const groupDocumentRoleContext = await invoke('mindnprogress_get_group_context', { groupId: groupTestId })
    assert.equal(groupDocumentRoleContext.guide.role, 'document-coordinator')
    assert.equal(groupDocumentRoleContext.guide.documentCoordinator, DOCUMENT_COORDINATOR_INSTRUCTION)
    assert.equal(groupDocumentRoleContext.guide.approval, undefined)
    assert.match(groupDocumentContext.selection.aiWorkCoordination.childDelegation.instruction, /후보 조회 → contextHealth 평가 확인/)
    ...
    assert.equal(coordinatorAgain.selection.workflow.writePolicy, 'approval-required')
    const coordinatorRoleContext = await invoke('mindnprogress_get_group_context', { groupId: groupTestId })
    assert.equal(coordinatorRoleContext.guide.executionApproval, AI_EXECUTION_APPROVAL_INSTRUCTION)
    assert.equal(coordinatorRoleContext.guide.approval, GROUP_APPROVAL_INSTRUCTION)
