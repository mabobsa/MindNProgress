...
export function AiConversationDialog({ documentId, cardId, cardTitle, onClose }: {
  documentId: string
  cardId: string
  cardTitle: string
  onClose: () => void
  ...
  const [workspace, setWorkspace] = useState('C:\\Git\\MindNProgress')
      ...
      <section className="ai-dialog" role="dialog" aria-modal="true" aria-label="AI 대화 시작 옵션">
        ...
        {loading ? <div className="ai-dialog-message">AionUi의 새 채팅 옵션을 불러오는 중…</div> : error ? (
          ...
          <div className="ai-dialog-content">
            ...
            <label><span>작업공간</span><input value={workspace} onChange={(event) => setWorkspace(event.target.value)} placeholder="선택사항" /></label>
            <details open>
