...
export function DoorayResponseInbox({ response, onOpenConversation, onOpenCard }: {
  ...
  const job = response.jobs.find((entry) => entry.id === response.selectedId) ?? response.jobs[0]
  ...
  useEffect(() => {
    ...
    void requestJson<Options>(`/api/integrations/aionui/options${machine ? `?machineId=${encodeURIComponent(machine)}` : ''}`)
      .then((result) => {
        ...
        if (initial) saveSettings({ machineId: result.machineId, agentId: initial.id,
  ...
  const chooseAgent = (id: string) => {
    ...
    response.saveSettings({ machineId: options.machineId, agentId: next.id,
    ...
    <section className="dooray-response-inbox" aria-label="AI 대응">
      <div className="dooray-response-toolbar">
        <button type="button" aria-expanded={response.open} onClick={() => response.setOpen(!response.open)}>
          AI 대응 {response.jobs.length > 0 && `${response.jobs.length}건`}{running > 0 && ` · ${running}건 진행 중`}
        </button>
      ...
      {response.open && <div className="dooray-response-detail">
        ...
        {response.jobs.length > 0 ? <>
          <label className="dooray-response-picker">대응 기록<select value={job?.id ?? ''} onChange={(event) => response.setSelectedId(event.target.value)}>
            {response.jobs.map((entry) => <option key={entry.id} value={entry.id}>{doorayResponseStatus[entry.status] ?? entry.status} · {entry.subject}</option>)}
          </select></label>
          {job && <article aria-live="polite">
            ...
            <div className="dooray-response-actions">
              {job.conversationId && <button type="button" onClick={() => onOpenConversation(job)}>대화에서 이어가기</button>}
            ...
            {['proposal', 'needs-input', 'failed'].includes(job.status) && <form className="dooray-response-refine" onSubmit={(event) => {
              ...
              <button type="submit" disabled={refining || !hint.trim()}>{refining ? '전달 중…' : '다시 제안받기'}</button>
            </form>}
          </article>}
        </> : response.pendingKeys.size === 0 && <p>참조 항목의 ‘AI 대응 제안’을 누르면 담당 탐색과 제안을 시작합니다.</p>}
      </div>}
    </section>
  )
}
