...
test('Dooray 승인은 담당 카드가 없어도 새 대화를 열고 승인 근거 확인 전문을 보존한다', () => {
  ...
  assert.match(prompt, /서버 승인과 허용·제외 범위를 확인/)
...
test('전문은 대상 식별자와 편집자 요청을 그대로 담는다', () => {
  ...
  assert.ok(prompt.includes('응답을 받지 못한 시도만 다시 호출'))
  assert.ok(prompt.includes('성공 뒤에는'))
  assert.ok(prompt.includes('get_context`를 반복하지'))
