...
for (const groupCoordinator of [false, true]) test(`재시작 후 ${groupCoordinator ? '같은 문서의 그룹 총괄' : '일반 상위'} AI의 수신·자동 보고와 전문 역할을 보존한다`, { timeout: 45_000 }, async () => {
  ...
  try {
    ...
    assert.equal(dispatchRequests[0].instruction.includes(AI_EXECUTION_APPROVAL_INSTRUCTION), false)
    assert.equal(dispatchRequests[0].instruction.includes('역할: group coordinator'), groupCoordinator)
