...
test('그룹 기획 관리, 문서 지시와 과거 루트 위임은 범위·동시 실행·복구 경계를 유지한다', { timeout: 60000 }, async () => {
  ...
  try {
    ...
    assert.match(recoveryCall.instruction, /원래 지시를 반복하지 말고 미완료 부분만/)
    assert.match(recoveryCall.instruction, /분석·제안 위임은 계속 읽기 전용/)
    ...
    assert.match(retriedWake.instruction, /최신 대화 응답으로 대체하지 않았/)
    ...
    assert.match(instructionCall.instruction, /worker 위임이나 작업공간 배정이 아닙니다/)
    assert.match(instructionCall.instruction, /문서 내부의 실제 하위 업무 카드에 AI 위임/)
    assert.match(instructionCall.instruction, /기본 대상: `group-parent`/)
    assert.match(instructionCall.instruction, /명시적 대체: `group-review-target`/)
    assert.match(instructionCall.instruction, /현재 유효 대상: `group-review-target`/)
    assert.match(instructionCall.instruction, /과거 지시·AION_SESSION_MESSAGE·reply_to·기억/)
