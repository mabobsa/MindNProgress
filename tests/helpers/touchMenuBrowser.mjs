...
export async function touchMenuBrowser({ base, command, evaluate, until, api }) {
  ...
  assert.equal(await evaluate(`document.querySelector('.react-flow__node[data-id="touch-parent"].selected')!==null`), true, '세부정보를 닫아도 카드 선택 유지')
  await evaluate(`globalThis.__mnpCardClickBlocker=(event)=>{if(event.target instanceof Element&&event.target.closest('.react-flow__node[data-id="touch-parent"]'))event.stopImmediatePropagation()};document.addEventListener('click',globalThis.__mnpCardClickBlocker,true)`)
  await tap(parent)
  await until(() => evaluate(`document.querySelector('#node-inspector-panel.mobile-open')!==null`), '같은 카드 재탭으로 세부정보 다시 열기 실패')
  await evaluate(`document.removeEventListener('click',globalThis.__mnpCardClickBlocker,true);delete globalThis.__mnpCardClickBlocker`)
  await evaluate(`document.querySelector('button[aria-label="모바일 패널 닫기"]')?.click()`)
